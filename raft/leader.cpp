...
std::tuple<int, int> client(int port, int follower_id, const char* ip, const char* my_ip) {
  hostip = gethostbyname("localhost");
  auto sending_fd = -1;
  auto fd = connect_to_the_server(port, ip, sending_fd, follower_id, my_ip);
...
int main(void) {
  auto [sending_socket_f1, listening_socket_f1] =
      client(follower_1_port, follower_1_id, follower_ip_1.c_str(), leader_ip.c_str());
  ...
  auto [sending_socket_f2, listening_socket_f2] =
      client(follower_2_port, follower_2_id, follower_ip_2.c_str(), leader_ip.c_str());
